...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	// Расчет средней скорости (км/ч)
	avgSpeed := meanSpeed(action, duration)

	// Формула расчета потраченных калорий:

	calories := ((float64(runningCaloriesMeanSpeedMultiplier) * avgSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	if duration == 0 {
		return 0 // исключаем деление на ноль
	}

	// Рассчитываем среднюю в метрах в секунду
	...
	// Формула расчета потраченных калорий:
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedSec, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	// Рассчитываем среднюю скорость плавания
